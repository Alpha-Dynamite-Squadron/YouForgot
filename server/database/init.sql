...
-- Mon 16 Nov 2020 12:45:39 AM PST
...
CREATE TABLE IF NOT EXISTS `yfdb`.`User` (
  ...
  `profileRating` INT NULL DEFAULT 100,
...
CREATE TABLE IF NOT EXISTS `yfdb`.`SectionInstance` (
  ...
  `academicTerm` VARCHAR(7) NOT NULL,
...
BEGIN
    IF NEW.Grade <> OLD.Grade THEN
        UPDATE Post SET iForgotCount = (SELECT AVG(Grade) AS newAverage FROM PostAssociation WHERE assignmentID = NEW.assignmentID) WHERE assignmentID = NEW.assignmentID;
    END IF;
END$$
